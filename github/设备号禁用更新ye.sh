...
generate_new_config() {
    ...
    # 修改现有文件
    modify_or_add_config "telemetry.machineId" "$machine_id" "$STORAGE_FILE" || exit 8
    modify_or_add_config "telemetry.macMachineId" "$mac_machine_id" "$STORAGE_FILE" || exit 9
    modify_or_add_config "telemetry.devDeviceId" "$device_id" "$STORAGE_FILE" || exit 10
    modify_or_add_config "telemetry.sqmId" "$sqm_id" "$STORAGE_FILE" || exit 11
...
main() {
    ...
    echo -e "666 zcDog老板"
    echo
    echo -e "${YELLOW}[重要提示]${NC} 仅支持 Cursor v0.45.x"
    echo
